...
def test_large_workstation_uses_slow_two_profile_batches() -> None:
    policy = FarmLaunchPolicy.for_total_memory(96 * GIB)

    assert policy.batch_size == 2
    assert policy.profile_interval_seconds == 6.0
    assert policy.batch_pause_seconds == 25.0
    assert policy.resource_constrained_interval_seconds == 12.0
    assert policy.min_available_memory_bytes >= 16 * GIB


def test_mid_range_machine_uses_slow_two_profile_batches() -> None:
    policy = FarmLaunchPolicy.for_total_memory(32 * GIB)

    assert policy.batch_size == 2
    assert policy.profile_interval_seconds == 7.0
    assert policy.batch_pause_seconds == 30.0
...
def test_low_memory_machine_opens_one_profile_per_batch() -> None:
    ...
    assert policy.profile_interval_seconds == 10.0
