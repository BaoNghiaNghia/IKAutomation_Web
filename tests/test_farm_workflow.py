...
def test_already_selected_team_advances_without_clicking_the_row_again() -> None:
    farm = FarmWorkflow(resource_order=("iron", "stone", "wood", "food"))
    farm.decide(FarmGameState.WORLD_MAP, ready_teams=(1, 2))
    farm.decide(FarmGameState.RESOURCE_SEARCH, target_verified=True)
    farm.decide(FarmGameState.RESOURCE_POPUP, target_verified=True)

    decision = farm.decide(FarmGameState.TEAM_SELECTION, team_selected=True)

    assert decision.step == FarmStep.DISPATCH
