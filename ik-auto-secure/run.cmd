...
"%VENV_PYTHON%" -c "import PySide6, playwright, pytest, qfluentwidgets, qrcode, winrt.windows.foundation, winrt.windows.security.credentials.ui" >nul 2>nul
if not errorlevel 1 goto :check_config
echo Installing required packages for the first run...
"%VENV_PYTHON%" -m pip install --disable-pip-version-check "PySide6>=6.8,<7" "PySide6-Fluent-Widgets>=1.7,<2" "playwright>=1.54,<2" "qrcode[pil]>=8,<9" "pytest>=8,<10" "winrt-Windows.Foundation>=3.2,<4" "winrt-Windows.Security.Credentials.UI>=3.2,<4" || goto :failed
