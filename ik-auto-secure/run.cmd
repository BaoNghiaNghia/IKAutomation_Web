...
"%VENV_PYTHON%" -c "import PySide6, playwright, pytest, qfluentwidgets" >nul 2>nul
if not errorlevel 1 goto :check_config
echo Installing required packages for the first run...
"%VENV_PYTHON%" -m pip install --disable-pip-version-check "PySide6>=6.8,<7" "PySide6-Fluent-Widgets>=1.7,<2" "playwright>=1.54,<2" "pytest>=8,<10" || goto :failed
