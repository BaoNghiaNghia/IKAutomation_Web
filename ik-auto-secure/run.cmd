...
"%VENV_PYTHON%" -c "import playwright, pytest, ttkbootstrap" >nul 2>nul
if not errorlevel 1 goto :check_config
echo Installing required packages for the first run...
"%VENV_PYTHON%" -m pip install --disable-pip-version-check "playwright>=1.54,<2" "pytest>=8,<10" "ttkbootstrap>=1.10,<2" || goto :failed
