...
class Dashboard(QWidget):
    ...
    def _build(self) -> None:
        ...
        accounts = self._card(); al = QVBoxLayout(accounts); al.addWidget(StrongBodyLabel("Tài khoản Chrome")); al.addWidget(self._muted("Mỗi tài khoản có một phiên browser riêng, lưu cục bộ.")); actions = QHBoxLayout(); actions.setSpacing(7); manage = PushButton("Quản lý tài khoản"); manage.setStyleSheet("QPushButton { background:#ffffff; border:1px solid #8ad7d9; color:#087f8c; } QPushButton:hover { background:#effcfb; border-color:#0ea5a5; }"); manage.clicked.connect(self._manage_accounts); open_all = PrimaryPushButton("Mở tất cả"); open_all.setStyleSheet("QPushButton { background:#3b82f6; border:1px solid #3b82f6; color:#ffffff; } QPushButton:hover { background:#2563eb; border-color:#2563eb; }"); open_all.clicked.connect(self._open_all_and_arrange); stop_all = self._icon_button(FIF.CLOSE, "Dừng tất cả profile đang chạy"); stop_all.setStyleSheet("QToolButton { background:#ef4444; border:1px solid #ef4444; border-radius:8px; color:#ffffff; } QToolButton:hover { background:#dc2626; border-color:#dc2626; }"); stop_all.clicked.connect(self.runner.stop_all); actions.addWidget(manage, 1); actions.addWidget(open_all, 1); actions.addWidget(stop_all); al.addLayout(actions); ll.addWidget(accounts)
